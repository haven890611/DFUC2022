...
from utils.dataloader1_all import get_loader, test_dataset
...
best = 0.
best_flag = True
def train(train_loader, test_loader, model, optimizer, optimizer1, epoch):
    model.train()
    ...
    meanloss, mean_acc, mean_dice, mean_iou, mean_fps = test(model, test_loader)
    if mean_dice > best:
...
train_save = 'crop_all_iou_focal_k15_jitter0.2'
...
# %%
trainset = get_loader(train_image_root, train_gt_root, batchsize=batch_size, img_size=384, group=group, jitter=0.2)
...
for epoch in range(1, epochs + 1):
    if best_flag:
        best_flag = False
        val_imgs, val_gts = trainset.split()
        testset = test_dataset(val_imgs, gts=val_gts, testsize=384)
        meanloss, mean_acc, mean_dice, mean_iou, mean_fps = test(model, testset)
        best = mean_dice
        print('[Next target]  Dice: {:0.4f}'.format(best))
        
    train(train_loader, testset, model, optimizer, optimizer1, epoch)
