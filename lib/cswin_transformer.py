...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
                 num_heads=12, mlp_ratio=4., branch=False, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        #if branch:
        self.norm3 = norm_layer(curr_dim//2)
        self.norm2 = norm_layer(curr_dim//4)
        self.norm1 = norm_layer(curr_dim//8)
        # Classifier head
    ...
    def forward_features(self, x):
        ...
        return [self.norm1(out_branch[0]), self.norm2(out_branch[1]), self.norm3(out_branch[2]), self.norm(out_branch[3])]
...
@register_model
def CSWin_64_12211_tiny_224(pretrained=False, branch=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=64, depth=[1,2,21,1],
        split_size=[1,2,7,7], num_heads=[2,4,8,16], mlp_ratio=4., branch=branch, **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model

@register_model
def CSWin_64_24322_small_224(pretrained=False, branch=False, **kwargs):
...
@register_model
def CSWin_96_24322_base_224(pretrained=False, branch=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=96, depth=[2,4,32,2],
        split_size=[1,2,7,7], num_heads=[4,8,16,32], mlp_ratio=4., branch=branch, **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model

@register_model
def CSWin_144_24322_large_224(pretrained=False, branch=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=144, depth=[2,4,32,2],
        split_size=[1,2,7,7], num_heads=[6,12,24,24], mlp_ratio=4., branch=branch, **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model
...
@register_model
def CSWin_96_24322_base_384(pretrained=False, branch=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=96, depth=[2,4,32,2],
        split_size=[1,2,12,12], num_heads=[4,8,16,32], mlp_ratio=4., branch=branch, **kwargs)
    model.default_cfg = default_cfgs['cswin_384']
    return model

@register_model
def CSWin_144_24322_large_384(pretrained=False, branch=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=144, depth=[2,4,32,2],
        split_size=[1,2,12,12], num_heads=[6,12,24,24], mlp_ratio=4., branch=branch, **kwargs)
    model.default_cfg = default_cfgs['cswin_384']
    return model
