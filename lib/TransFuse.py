...
class ChannelPool(nn.Module):
    ...
    def forward(self, x):
        return torch.cat((torch.amax(x, dim=1, keepdim=True), torch.mean(x, 1).unsqueeze(1)), dim=1)
...
class BiFusion_block(nn.Module):
    def __init__(self, ch_1, ch_2, r_2, ch_int, ch_out, drop_rate=0.):
        ...
        self.W = Conv(ch_int, ch_int, 3, bn=True, relu=False)
    ...
    def forward(self, g, x):
        # bilinear pooling
        W_g = self.W_g(g) # W_2^i
        W_x = self.W_x(x) # W_1^i
        bp = self.W(W_g*W_x) # b^i
...
class TransFuse_S(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        self.final_x = nn.Sequential(
            Conv(256, 64, 1, bn=True, relu=False),
            Conv(64, 64, 3, bn=True, relu=False),
    ...
    def forward(self, imgs, labels=None):
        ...
        x_b = x_b.view(x_b.shape[0], -1, int(h / 16), int(w / 16))
        x_b = self.drop(x_b)
        # t^0

        x_b_1 = self.up1(x_b)
        x_b_1 = self.drop(x_b_1) # t^1

        x_b_2 = self.up2(x_b_1)  # transformer pred supervise here
        x_b_2 = self.drop(x_b_2) # t^2
        ...
        x_u_2 = self.resnet.layer1(x_u) # g^2
        x_u_2 = self.drop(x_u_2)

        x_u_1 = self.resnet.layer2(x_u_2) # g^1
        x_u_1 = self.drop(x_u_1)

        x_u = self.resnet.layer3(x_u_1) # g^0
        x_u = self.drop(x_u) 

        # joint path
        x_c = self.up_c(x_u, x_b) # f^0

        x_c_1_1 = self.up_c_1_1(x_u_1, x_b_1) # f^1
        x_c_1 = self.up_c_1_2(x_c, x_c_1_1) # f^hat1

        x_c_2_1 = self.up_c_2_1(x_u_2, x_b_2) # f^2
        x_c_2 = self.up_c_2_2(x_c_1, x_c_2_1) # f^hat2

        # decoder part
        map_x = F.interpolate(self.final_x(x_c), scale_factor=16, mode='bilinear', align_corners=True)  # f^0
        map_1 = F.interpolate(self.final_1(x_b_2), scale_factor=4, mode='bilinear', align_corners=True) # t^2
        map_2 = F.interpolate(self.final_2(x_c_2), scale_factor=4, mode='bilinear', align_corners=True) # f^hat2
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2=None):
        ...
        if x2 is not None:
            ...
            pad_y_left = torch.divide(diffY, 2, rounding_mode="floor")
            pad_y_right = diffY - pad_y_left
            
            diffX = torch.tensor([x2.size()[3] - x1.size()[3]])
            pad_x_left = torch.divide(diffX, 2, rounding_mode="floor")
            pad_x_right = diffX - pad_x_left
            
            # left, right, top, bottom
            x1 = F.pad(x1, [pad_x_left, pad_x_right,
